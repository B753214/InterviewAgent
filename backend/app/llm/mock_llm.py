...
MOCK_ASSESSMENT = {
    ...
    "suggested_review": ["系统设计取舍", "性能优化量化"],
    "memory_updates": [
        {
            "topic": "系统设计",
            "category": "architecture",
            "performance": "adequate",
            "evidence": "mock assessment fallback",
        },
        {
            "topic": "性能优化",
            "category": "backend",
...
def mock_interviewer_question(action: str, current_topic: str | None = None) -> str:
    ...
    if action == "initial_question":
        question = random.choice(MOCK_QUESTIONS["initial_question"])
        if topic:
            return f"{question}（当前主题：{topic}）"
        return question
    if action == "follow_up":
        question = random.choice(MOCK_QUESTIONS["follow_up"])
        if topic:
            return f"关于「{topic}」：{question}"
        return question
    question = random.choice(MOCK_QUESTIONS["switch_topic"])
    if topic:
        return f"{question}（建议聚焦：{topic}）"
    return question
...
def mock_router_decision(
    ...
    if follow_up_count >= 2:
        return {
            ...
            "quality": "adequate",
            "next_topic": None,
            "reason": f"已追问{follow_up_count}次",
        }
