...
export const api = {
  ...
  uploadMaterialPdf: (data: { name: string; file: File }) => {
    ...
    return upload<{
      id: string; name: string; type: string; embedding_status: string;
      chunk_count: number; processing_error?: string;
    }>("/materials/upload", formData);
  },

  listMaterials: () => request<{
    id: string; name: string; type: string; enabled: boolean; chunk_count: number; embedding_status: string;
    processing_error?: string; created_at: string;
  }[]>("/materials"),

  getMaterial: (id: string) => request<{
    id: string; name: string; type: string; raw_text: string; chunk_count: number;
  ...
  listMemories: (sortBy = "mastery_score") =>
    request<{
      id: string; topic: string; category: string; mastery_score: number; exposure_count: number;
